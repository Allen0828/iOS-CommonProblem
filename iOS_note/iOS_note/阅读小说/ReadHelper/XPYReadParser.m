...
/// 本地书分章节正则表达
static NSString * const kParseLocalBookPattern = @"(\\s+?)([#☆、【0-9]{0,10})(第[0-9零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟\\s]{1,10}[章节集卷])(.*)";
...
+ (NSArray<NSString*>*)rectsWithRange:(NSRange)range content:(NSString *)content frameRef:(CTFrameRef)frameRef {
    ...
    for (int i = 0; i < count; i++) {
        ...
        [rects addObject:NSStringFromCGRect(contentRect)];
    }
    return [rects copy];
}
