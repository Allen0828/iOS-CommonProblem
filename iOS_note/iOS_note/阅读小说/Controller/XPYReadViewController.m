...
- (void)updateTouch:(NSSet<UITouch *> *)touches touchType:(TouchType)type {
    ...
    CGPoint windowPoint = [touch locationInView:self.view];
    if (!CGPointEqualToPoint(CGPointZero, windowPoint)) {
        CGPoint point = [self.view convertPoint:windowPoint toView:self.readView];
        [self.readView updateTouchType:type selectPoint:point windowPoint:windowPoint];
    }
}
